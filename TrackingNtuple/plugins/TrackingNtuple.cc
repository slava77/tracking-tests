...
private:
  ...
  float bsp_sigmaz;
  //seeds
  std::vector<float> see_lh_px       ;
  std::vector<float> see_lh_py       ;
  std::vector<float> see_lh_pz       ;
  std::vector<float> see_lh_pt       ;
  std::vector<float> see_lh_eta      ;
  std::vector<float> see_lh_phi      ;
  std::vector<float> see_lh_x      ;
  std::vector<float> see_lh_y      ;
  std::vector<float> see_lh_z      ;
  std::vector<float> see_pca_px       ;
  std::vector<float> see_pca_py       ;
  std::vector<float> see_pca_pz       ;
  std::vector<float> see_pca_pt       ;
  std::vector<float> see_pca_eta      ;
  std::vector<float> see_pca_phi      ;
  std::vector<float> see_pca_dxy      ;
  std::vector<float> see_pca_x      ;
  std::vector<float> see_pca_y      ;
  std::vector<float> see_pca_z      ;
  std::vector<float> see_pca_dz       ;
  std::vector<float> see_pca_ptErr    ;
  std::vector<float> see_pca_etaErr   ;
  std::vector<float> see_pca_phiErr   ;
  std::vector<float> see_pca_dxyErr   ;
  std::vector<float> see_pca_dzErr    ;
...
void TrackingNtuple::clearVariables() {
  ...
  see_pca_dz      .clear();
  see_pca_ptErr   .clear();
  see_pca_etaErr  .clear();
  see_pca_phiErr  .clear();
  see_pca_dxyErr  .clear();
  see_pca_dzErr   .clear();
  see_chi2    .clear();
  see_q       .clear();
  see_nValid  .clear();
...
void TrackingNtuple::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned int itsd=0;itsd<seedTags_.size();++itsd) {
    ...
    for(TrajectorySeedCollection::const_iterator itSeed = seeds->begin(); itSeed != seeds->end(); ++itSeed,++seedCount) {
      ...
      see_lh_px      .push_back( state.globalParameters().momentum().x() );
      see_lh_py      .push_back( state.globalParameters().momentum().y() );
      see_lh_pz      .push_back( state.globalParameters().momentum().z() );
      see_lh_pt      .push_back( pt );
      see_lh_eta     .push_back( eta );
      see_lh_phi     .push_back( phi );
      see_lh_x      .push_back( state.globalParameters().position().x() );
      see_lh_y      .push_back( state.globalParameters().position().y() );
      see_lh_z      .push_back( state.globalParameters().position().z() );
      ...
      see_pca_px      .push_back( matchedTrackPointer->px() );
      see_pca_py      .push_back( matchedTrackPointer->py() );
      see_pca_pz      .push_back( matchedTrackPointer->pz() );
      see_pca_pt      .push_back( matchedTrackPointer->pt() );
      see_pca_eta     .push_back( matchedTrackPointer->eta() );
      see_pca_phi     .push_back( matchedTrackPointer->phi() );
      see_pca_x       .push_back( matchedTrackPointer->vx() );
      see_pca_y       .push_back( matchedTrackPointer->vy() );
      see_pca_z       .push_back( matchedTrackPointer->vz() );
      see_pca_dxy     .push_back(matchedTrackPointer->dxy(bs.position()));
      see_pca_dz      .push_back(matchedTrackPointer->dz(bs.position()));
      see_pca_ptErr   .push_back(matchedTrackPointer->ptError());
      see_pca_etaErr  .push_back(matchedTrackPointer->etaError());
      see_pca_phiErr  .push_back(matchedTrackPointer->phiError());
      see_pca_dxyErr  .push_back(matchedTrackPointer->dxyError());
      see_pca_dzErr   .push_back(matchedTrackPointer->dzError());
...
void TrackingNtuple::beginJob() {
  ...
  //seeds
  t->Branch("see_lh_px"       , &see_lh_px      );
  t->Branch("see_lh_py"       , &see_lh_py      );
  t->Branch("see_lh_pz"       , &see_lh_pz      );
  t->Branch("see_lh_pt"       , &see_lh_pt      );
  t->Branch("see_lh_eta"      , &see_lh_eta     );
  t->Branch("see_lh_phi"      , &see_lh_phi     );
  t->Branch("see_lh_x"       , &see_lh_x      );
  t->Branch("see_lh_y"       , &see_lh_y      );
  t->Branch("see_lh_z"       , &see_lh_z      );
  t->Branch("see_pca_px"       , &see_pca_px      );
  t->Branch("see_pca_py"       , &see_pca_py      );
  t->Branch("see_pca_pz"       , &see_pca_pz      );
  t->Branch("see_pca_pt"       , &see_pca_pt      );
  t->Branch("see_pca_eta"      , &see_pca_eta     );
  t->Branch("see_pca_phi"      , &see_pca_phi     );
  t->Branch("see_pca_x"       , &see_pca_x      );
  t->Branch("see_pca_y"       , &see_pca_y      );
  t->Branch("see_pca_z"       , &see_pca_z      );
  t->Branch("see_pca_dxy"      , &see_pca_dxy     );
  t->Branch("see_pca_dz"       , &see_pca_dz      );
  t->Branch("see_pca_ptErr"    , &see_pca_ptErr   );
  t->Branch("see_pca_etaErr"   , &see_pca_etaErr  );
  t->Branch("see_pca_phiErr"   , &see_pca_phiErr  );
  t->Branch("see_pca_dxyErr"   , &see_pca_dxyErr  );
  t->Branch("see_pca_dzErr"    , &see_pca_dzErr   );
